...
```yaml
- name: myapp
  server: { prod: hyperion, sham: hyperion-sham }
  app_db: [zban_myapp]
  sup_db: [myapp]
```
...
hyperion-sham:
  host: "hyperion-sham.example.com"
  port: 3306
  user: "reconciler"
...
- `tp-{{name}}-adm` for program `myapp` → role `tp-myapp-adm`, with grants scoped to `myapp`'s databases only
...
app_db:
  ...
  'dst_%': ["select", "dml", "ddl"]       # LIKE pattern → GRANT ... ON schema.dst_%
```
...
```yaml
app_db: ["zban_qa%"]   # expands to all databases matching zban_qa% on that server
```
