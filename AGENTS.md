...
- LIKE pattern like `'src_%'` — table pattern → `GRANT ... ON schema.src_%`
...
- `{{name}}-adm` for program `myapp` → role `myapp-adm`, grants on `myapp`'s `app_db` and `sup_db` only
...
- Example: `app_db: ["appname_qa%"]` expands to all databases matching `appname_qa%` on that server
  ...
  "servers": [
    {
      ...
      "roles": ["myapp-adm", "myapp-ro"],
      "grants": [
        {"role": "myapp-adm", "database": "*", "table": "", "privileges": ["PROCESS"]},
        {"role": "myapp-adm", "database": "appname_myapp", "table": "*", "privileges": ["SELECT", "INSERT"]}
      ],
      "statements": [
        {"sql": "CREATE ROLE 'myapp-adm'", "type": "create_role", "role": "myapp-adm"}
      ],
      "checksum": "sha256-hex"
    }
  ]
