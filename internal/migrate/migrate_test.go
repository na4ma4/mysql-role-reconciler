...
func TestWriteAndReadPlanFile(t *testing.T) {
	...
	if plan.Servers[0].Server != "rdsserver1" {
		t.Errorf("expected server rdsserver1, got %s", plan.Servers[0].Server)
	}
...
func TestStateStore(t *testing.T) {
	...
	_, exists := stateStore.Get("rdsserver1")
	if exists {
		t.Error("expected no state for rdsserver1 initially")
	}

	if err := stateStore.Update(ctx, "rdsserver1", migrate.ServerState{
		Checksum:    "abc123",
	...
	state, exists := stateStore2.Get("rdsserver1")
	if !exists {
		t.Fatal("expected state for rdsserver1 after update")
	}
...
func TestWriteAndReadHistory(t *testing.T) {
	...
	if entries[0].Server != "rdsserver1" {
		t.Errorf("expected server rdsserver1, got %s", entries[0].Server)
	}
	...
	if yamlEntry.Server != "rdsserver1" {
		t.Errorf("YAML entry: expected server rdsserver1, got %s", yamlEntry.Server)
	}
}
