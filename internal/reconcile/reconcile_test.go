...
func TestLikeMatch(t *testing.T) {
	...
	tests := []struct {
		...
		{"objects", "obj%", true},
		{"dst_table1", "dst_%", true},
		{"ext_schema", "ext_%", true},
		{"other", "dst_%", false},
		{"abc", "a%c", true},
		{"ac", "a%c", true},
		{"ab", "a_b", false},
...
func TestBuildDesiredState(t *testing.T) {
	...
	progs := config.ProgramsFile{
		{
			Name:   "app1",
			Server: map[string]string{"prod": "hyperion"},
			AppDB:  []string{"db_app1"},
			SupDB:  []string{"sup_app1"},
		},
	}

	state := reconcile.BuildDesiredState("hyperion", "prod", cfg, progs)

	if state.Server != "hyperion" {
		t.Errorf("expected server hyperion, got %s", state.Server)
	}
...
func TestBuildDesiredState_DisabledProgramExcluded(t *testing.T) {
	...
	for _, g := range state.Grants {
		if g.Database == "zban_trslot" {
			t.Errorf("disabled program's database zban_trslot should not appear in grants, but found: %+v", g)
		}
	}
	for _, r := range state.Roles {
		if r == "tp-trslot-ro" {
			t.Error("disabled program's role tp-trslot-ro should not appear in roles")
		}
	}
}
...
func TestBuildDesiredState_TemplatedRoles(t *testing.T) {
	...
	// tp-app1-adm should have: server-level + db_app1.* (app_db) + sup_app1.* (sup_db) = 3 grants
	admApp1Grants := 0
	for _, g := range state.Grants {
		if g.Role == "tp-app1-adm" {
			admApp1Grants++
		}
	}
	if admApp1Grants != 3 {
		t.Errorf("expected 3 grants for tp-app1-adm, got %d", admApp1Grants)
	}

	// tp-app2-adm should NOT have db_app1 in its grants
	for _, g := range state.Grants {
		if g.Role == "tp-app2-adm" && g.Database == "db_app1" {
			t.Errorf("tp-app2-adm should not have grants on db_app1")
		}
	}
}
...
func TestBuildDesiredState_TableLevelGrants(t *testing.T) {
	t.Parallel()
	// tp-{{name}}-mnt scenario: role has specific TABLE overrides in app_db
	cfg := &config.Config{
		PermissionSets: map[string][]string{
			"usage":       {"USAGE"},
			"select":      {"SELECT"},
	...
	if _, ok := roleSet["tp-myapp-mnt"]; !ok {
		t.Fatalf("expected role tp-myapp-mnt, got %v", state.Roles)
	}

	// Collect grants for tp-myapp-mnt as schema.table → privileges
	grantMap := make(map[string][]string)
	for _, g := range state.Grants {
		if g.Role == "tp-myapp-mnt" {
	...
	// app_db schema wildcard: zban_myapp.* → select, create_temp
	if perms, ok := grantMap["zban_myapp.*"]; !ok {
		t.Error("expected grant on zban_myapp.* (app_db wildcard tables)")
	} else if len(perms) != 2 {
		t.Errorf("expected 2 perms on zban_myapp.*, got %d: %v", len(perms), perms)
	}

	// Table-level grants within the app_db schema
	for _, table := range []string{"objects", "operations", "users"} {
		key := "zban_myapp." + table
	...
	// sup_db table pattern: myapp.dst_% → select, dml, ddl
	if perms, ok := grantMap["myapp.dst_%"]; !ok {
		t.Error("expected grant on myapp.dst_% (sup_db pattern tables)")
	} else if len(perms) != 7 { // SELECT + INSERT + UPDATE + DELETE + CREATE + ALTER + DROP
		t.Errorf("expected 7 perms on myapp.dst_%%, got %d: %v", len(perms), perms)
	}
}
...
func TestDiff_UsageSubsumedByOtherPrivilege(t *testing.T) {
	...
	desired := &reconcile.DesiredState{
		Server: "hyperion",
		Roles:  []string{"tp-myapp-adm"},
		Grants: []reconcile.DesiredGrant{
			{Role: "tp-myapp-adm", Database: "*", Table: "", Privileges: []string{"USAGE", "PROCESS"}},
		},
	}
	...
	actual := &mysql.ActualState{
		Roles: []string{"tp-myapp-adm"},
		Grants: []mysql.Grant{
			{Role: "tp-myapp-adm", Database: "*", Table: "", Grants: []string{"PROCESS"}},
		},
	}
...
func TestExpandDatabasePatterns_PatternWithSpecificTable(t *testing.T) {
	...
	state := &reconcile.DesiredState{
		Server: "test",
		Grants: []reconcile.DesiredGrant{
			{Role: "ro", Database: "zban_qa%", Table: "players", Privileges: []string{"SELECT"}},
		},
	}
	dbNames := []string{"zban_qa1", "zban_qa2", "zban_prod1", "otherdb"}
	reconcile.ExpandDatabasePatterns(state, dbNames)

	if len(state.Grants) != 2 {
		t.Fatalf("expected 2 grants (one per matching DB), got %d: %+v", len(state.Grants), state.Grants)
	}
	if state.Grants[0].Database != "zban_qa1" || state.Grants[0].Table != "players" {
		t.Errorf("expected grant on zban_qa1.players, got %s.%s", state.Grants[0].Database, state.Grants[0].Table)
	}
	if state.Grants[1].Database != "zban_qa2" || state.Grants[1].Table != "players" {
		t.Errorf("expected grant on zban_qa2.players, got %s.%s", state.Grants[1].Database, state.Grants[1].Table)
	}
}
...
func TestExpandDatabasePatterns_PatternWithWildcardTable(t *testing.T) {
	...
	state := &reconcile.DesiredState{
		Server: "test",
		Grants: []reconcile.DesiredGrant{
			{Role: "ro", Database: "zban_qa%", Table: "*", Privileges: []string{"SELECT"}},
		},
	}
	dbNames := []string{"zban_qa1", "zban_qa2"}
	reconcile.ExpandDatabasePatterns(state, dbNames)

	if len(state.Grants) != 2 {
		t.Fatalf("expected 2 grants, got %d", len(state.Grants))
	}
	if state.Grants[0].Database != "zban_qa1" || state.Grants[0].Table != "*" {
		t.Errorf("expected zban_qa1.*, got %s.%s", state.Grants[0].Database, state.Grants[0].Table)
	}
	if state.Grants[1].Database != "zban_qa2" || state.Grants[1].Table != "*" {
		t.Errorf("expected zban_qa2.*, got %s.%s", state.Grants[1].Database, state.Grants[1].Table)
	}
}
...
func TestExpandDatabasePatterns_UnderscoreIsLiteral(t *testing.T) {
	...
	state := &reconcile.DesiredState{
		Server: "test",
		Grants: []reconcile.DesiredGrant{
			{Role: "ro", Database: "zban_qa_%", Table: "players", Privileges: []string{"SELECT"}},
		},
	}
	dbNames := []string{"zban_qa1", "zban_qa_test", "zban_qa_prod"}
	reconcile.ExpandDatabasePatterns(state, dbNames)

	if len(state.Grants) != 2 {
		t.Fatalf("expected 2 grants (only zban_qa_test and zban_qa_prod), got %d: %+v", len(state.Grants), state.Grants)
	}
	for _, g := range state.Grants {
		if !strings.HasPrefix(g.Database, "zban_qa_") {
			t.Errorf("expected database to start with zban_qa_, got %q", g.Database)
		}
	}
}
...
func TestExpandDatabasePatterns_NoMatches(t *testing.T) {
	...
	state := &reconcile.DesiredState{
		Server: "test",
		Grants: []reconcile.DesiredGrant{
			{Role: "ro", Database: "zban_nonexistent%", Table: "players", Privileges: []string{"SELECT"}},
		},
	}
	dbNames := []string{"zban_qa1", "otherdb"}
	reconcile.ExpandDatabasePatterns(state, dbNames)
...
func TestExpandDatabasePatterns_Mixed(t *testing.T) {
	...
	dbNames := []string{"mydb", "zban_qa1", "zban_qa2", "otherdb"}
	reconcile.ExpandDatabasePatterns(state, dbNames)
