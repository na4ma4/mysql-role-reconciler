...
func TestBuildDesiredState_FunctionGrants(t *testing.T) {
	t.Parallel()
	cfg := &config.Config{
		PermissionSets: map[string][]string{
			"execute_procedure": {"EXECUTE"},
		...
		Roles: []config.RoleConfig{{
			...
			AppDBObjects: []config.RoleObjectConfig{{
				Type:       "function",
				Names:      []string{"get_user", "delete_expired"},
				Privileges: []string{"execute_procedure"},
			}},
		}},
	}
	...
	state := reconcile.BuildDesiredState("rdsserver1", "prod", cfg, progs)
	if len(state.Grants) != 2 {
		t.Fatalf("expected two function grants, got %d: %+v", len(state.Grants), state.Grants)
	}
	for _, grant := range state.Grants {
		if grant.ObjectType != "function" || grant.Database != "appname_myapp" {
			t.Errorf("unexpected function grant: %+v", grant)
		}
	}
	...
	for _, stmt := range stmts {
		if stmt.SQL != "GRANT EXECUTE ON FUNCTION `appname_myapp`.`"+stmt.Table+"` TO 'myapp-mnt'" {
			t.Errorf("unexpected function SQL: %s", stmt.SQL)
		}
	}
}
