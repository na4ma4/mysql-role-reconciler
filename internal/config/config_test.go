...
func TestMergePrograms_MergeServer(t *testing.T) {
	t.Parallel()
	fileProgs := config.ProgramsFile{
		{Name: "app1", Server: map[string]string{"prod": "rdsserver1"}},
	}

	cfgProgs := []config.ProgramConfig{
		{Name: "app1", Server: map[string]string{"sham": "rdsserver1-sham"}},
	}

	fileProgs = config.MergePrograms(fileProgs, cfgProgs)

	if fileProgs[0].Server["sham"] != "rdsserver1-sham" {
		t.Errorf("expected sham server mapping, got %v", fileProgs[0].Server)
	}
	if fileProgs[0].Server["prod"] != "rdsserver1" {
...
`
	...
	if _, ok := srvs["rdsserver1"]; !ok {
		t.Error("expected rdsserver1 server")
	}
...
`
	...
	// rdsserver1: enabled not specified, should default to true
	if !srvs["rdsserver1"].Enabled.Get() {
		t.Error("expected rdsserver1.Enabled to default to true")
	}

	// rdsserver2: enabled explicitly set to false
	if srvs["rdsserver2"].Enabled.Get() {
		t.Error("expected rdsserver2.Enabled to be false")
	}
}
...
`
	...
	if !srvs["rdsserver1"].Enabled.Get() {
		t.Error("expected rdsserver1.Enabled to be true")
	}
}
...
func TestExpandRole(t *testing.T) {
	...
	if expanded.Name != "myapp-adm" {
		t.Errorf("expected myapp-adm, got %s", expanded.Name)
	}
...
func TestExpandRolesForServer(t *testing.T) {
	t.Parallel()
	roles := []config.RoleConfig{
		{Name: "{{name}}-adm", Server: map[string][]string{"*": {"process"}}},
		{Name: "{{name}}-ro", Server: map[string][]string{"*": {"usage"}}},
	...
	progs := config.ProgramsFile{
		...
		{
			Name:   "app3",
			Server: map[string]string{"sham": "rdsserver1-sham"},
			AppDB:  []string{"db3"},
			SupDB:  []string{"sup3"},
		},
	}

	expanded := config.ExpandRolesForServer(roles, progs, "rdsserver1", "prod")

	// Should have: app1-adm, app1-ro, app2-adm, app2-ro, global_admin = 5
	if len(expanded) != 5 {
	...
	expected := []string{"app1-adm", "app1-ro", "app2-adm", "app2-ro", "global_admin"}
...
func TestFilterDisabledPrograms(t *testing.T) {
	t.Parallel()
	progs := config.ProgramsFile{
		{Name: "active", Server: map[string]string{"prod": "rdsserver1"}},
		{Name: "disabled", Server: map[string]string{"prod": "rdsserver1"}},
	}
...
`
	...
	if len(progs[0].AppDB) != 1 || progs[0].AppDB[0] != "appname_foobar" {
		t.Errorf("expected app_db [appname_foobar], got %v", progs[0].AppDB)
	}
	if progs[0].Server["prod"] != "rdsserver1" {
		t.Errorf("expected server.prod=rdsserver1, got %v", progs[0].Server)
	}
}
